            ...
            if ( empty( $dircontents['dirs'] ) && empty( $dircontents['files'] ) ) {
                ...
                if ( !empty( $dircontents['files'] ) ) {
                    ...
                    foreach ( $dircontents['files'] as $file ) {
                        ...
                        echo "<div class=\"file{$nowplaying}\">
                                <div class=\"filebutton\">{$counter}</div>
        ...
        if ( empty( $playlist ) ) {
            ...
            foreach ( $playlist as $link ) {
                ...
                $jslink = str_replace( "'", "\'", $link );
                 ...
                 $sql = "SELECT song_name FROM vote WHERE song_name='$song'";
                 $result = mysqli_query($conn, $sql);
                 if (mysqli_num_rows($result) < 1) {
                     $sql = "INSERT INTO vote (song_name, file_name) VALUES ('$song', '$file')";
            ...
            $sql = "SELECT id, song_name, file_name, vote_count FROM vote ORDER BY vote_count DESC";
            $result = mysqli_query($conn, $sql);
            if (mysqli_num_rows($result) > 0) {
                ...
                while($row = mysqli_fetch_assoc($result)) {
                    ...
                    $voteCount = $row["vote_count"] ? $row["vote_count"] : 0; 
                    ...
                    echo "<div class=\"file{$nowplaying}\">
                            ...
                            <div class=\"filebutton\" style='color: green;'> {$voteCount} </div>
                            <div class=\"filebutton\" onclick=\"moveInPlaylist('{$jslink}', -1);\"title=\"Move up\">&#x2191</div>
                            <div class=\"filebutton\" onclick=\"moveInPlaylist('{$jslink}', 1);\"title=\"Move down\">&#x2193</div>
                            <div class=\"filebutton\" onclick=\"removeFromPlaylist('{$jslink}');\" title='Remove from playlist'><a href='addVote.php?id={$row['id']}&status=delete'>&#x00d7</a></div>
                        </div>";    
                }
            }
                // echo "<div class=\"file{$nowplaying}\">
                ...
                //         <div class=\"filebutton\" onclick=\"moveInPlaylist('{$jslink}', -1);\"title=\"Move up\">&#x2191</div>
                //         <div class=\"filebutton\" onclick=\"moveInPlaylist('{$jslink}', 1);\"title=\"Move down\">&#x2193</div>
                //         <div class=\"filebutton\" onclick=\"removeFromPlaylist('{$jslink}');\" title=\"Remove from playlist\">&#x00d7</div>
                //     </div>";
...
function renderButtons() {
    ...
    if ( $viewmode == 'playlist' ) {
        ...
        <a href='addVote.php?status=clear' style="color: black; text-decoration: none"> <div class="button" onclick="clearPlaylist();"><span>Clear</span></div></a>
...
PLBUTTONS;
    ...
    <div class="buttons">
        ...
        <div class="separator"></div>
        <div class="button{$playlistactive}" onclick="goToPlaylist('default')" style="background: green; color:white">
            <a href="vote.php" style="background: green; color:white; text-decoration: none;" target="_blank"><span>Vote</span></a>
        </div>
    </div>
BUTTONS;
}
...
<div id="stickycontainer">
    ...
    <div id="" align="center" style="color:yellow; font-size: 40px;">TuneBox</div>
    <br />
        <div id="player">
            <div id="albumart"></div>
            <div id="song">
